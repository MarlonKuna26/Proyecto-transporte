    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {quickActions.map((a) => (
          <Link key={a.path} to={a.path} className="glass-card p-5 card-hover group">
            <div className={`w-12 h-12 rounded-xl ${a.color} flex items-center justify-center text-2xl mb-3 group-hover:scale-110 transition-transform shadow-blue`}>
              {a.icon}
            </div>
            <h3 className="font-semibold text-navy-900 text-sm md:text-base">{a.title}</h3>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-navy-900">Viajes recientes</h2>
          <Link to="/rides" className="text-primary-500 hover:text-primary-600 text-sm font-medium transition-colors">
            Ver todos →
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="glass-card p-8 text-center">
            ...
            <p className="text-dark-500">No hay viajes disponibles por ahora</p>
              ...
              <Link key={ride.id} to={`/rides?view=${ride.id}`} className="glass-card p-5 card-hover">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-navy-900 font-medium">{ride.originZone}</span>
                    <span className="text-dark-400">→</span>
                    <span className="text-navy-900 font-medium">{ride.destinationZone}</span>
                  </div>
                ...
                {ride.pricePerSeat > 0 && (
                  <p className="mt-2 text-primary-600 font-semibold text-sm">${ride.pricePerSeat.toLocaleString()}/persona</p>
                )}
              </Link>
            ))}
          </div>
