...
export const MyRequestsPage: React.FC = () => {
  ...
  const executeCancelRequest = async () => {
    ...
    try {
      ...
      setRequests(prev => prev.map(r => r.id === cancelRequestId ? { ...r, status: 'CANCELLED' as const } : r));
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-8 py-8 space-y-6" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      {loading ? (
        ...
        <div className="space-y-4 animate-fade-in">
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-white rounded-2xl p-6 border border-uber-gray-100 animate-pulse space-y-3">
        ...
        <div className="bg-white rounded-3xl p-12 text-center border border-uber-gray-100 shadow-uber-sm max-w-xl mx-auto my-6 animate-fade-in">
          ...
          <h3 className="text-xl font-bold text-black mb-2">No tienes solicitudes</h3>
          <p className="text-sm text-uber-gray-500 mb-6 max-w-sm mx-auto">
            Explora los viajes compartidos publicados en el campus y solicita unirte para viajar cómodo y seguro.
          </p>
          <Link
            to="/rides"
            ...
            return (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  {/* Left Column: Route, Date, Seats, Status */}
                  <div className="flex-1 space-y-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="text-xs text-uber-gray-400 font-bold uppercase tracking-wider leading-none">Origen</div>
                            <div className="font-bold text-black text-sm truncate mt-0.5 leading-normal">{ride.originZone}</div>
                            {ride.originDetail && <p className="text-xs text-uber-gray-500 font-medium truncate">{ride.originDetail}</p>}

                            <div className="h-3.5" />

                            <div className="text-xs text-uber-gray-400 font-bold uppercase tracking-wider leading-none">Destino</div>
                            <div className="font-bold text-black text-sm truncate mt-0.5 leading-normal">{ride.destinationZone}</div>
                            {ride.destinationDetail && <p className="text-xs text-uber-gray-500 font-medium truncate">{ride.destinationDetail}</p>}
                          </div>
                        </div>

                        {/* Date and Time */}
                        <div className="flex items-center gap-2 text-xs text-uber-gray-600 font-medium pl-6 bg-uber-gray-50/50 py-2 rounded-xl">
                          ...
                          <span>Salida: <span className="font-bold text-black">{ride.departureDate}</span> a las <span className="font-bold text-black">{ride.departureTime}</span></span>
                        </div>
                      </div>
                    ) : (
                      /* Minimal Loader for ride */
                      <div className="space-y-2 py-2">
                        <div className="h-3 bg-uber-gray-50 rounded w-1/2 animate-pulse" />
                        <div className="h-3 bg-uber-gray-50 rounded w-1/3 animate-pulse" />
                      </div>
                    )}
                    ...
                    {req.message && (
                      <div className="bg-uber-gray-50 border border-uber-gray-100 rounded-xl px-4 py-2.5 max-w-xl">
                        <span className="block text-[9px] text-uber-gray-400 font-bold uppercase tracking-wider mb-0.5">Tu mensaje</span>
                        <p className="text-xs text-uber-gray-700 font-medium leading-relaxed">"{req.message}"</p>
                      </div>
                    )}

                    {/* Reject reason warning banner */}
                    {req.status === 'REJECTED' && req.rejectReason && (
                      <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3 max-w-xl flex items-start gap-2.5 text-uber-red">
                        ...
                        <div>
                          <span className="block text-[9px] font-bold uppercase tracking-wider text-red-500">Motivo del rechazo</span>
                          <p className="text-xs font-semibold mt-0.5">{req.rejectReason}</p>
                        </div>
                      </div>
                    )}
                  </div>

                  {/* Right Column: Price summary & Cancel Button */}
                  <div className="flex flex-row md:flex-col md:items-end justify-between items-center md:justify-start gap-4 md:text-right shrink-0">
                    {ride && (
                      <div>
                        <div className="text-[10px] text-uber-gray-400 font-bold uppercase tracking-wider">Costo Estimado</div>
                        <div className="text-2xl font-black text-black mt-0.5">
                          ${(req.seatsRequested * ride.pricePerSeat).toLocaleString()}
                        </div>
                        <div className="text-[10px] text-uber-gray-500 font-medium">
                          ${ride.pricePerSeat.toLocaleString()} x {req.seatsRequested} asiento{req.seatsRequested !== 1 ? 's' : ''}
                        </div>
                      </div>
                    )}

                    {/* Actions: Cancel request if pending */}
                    {req.status === 'PENDING' && (
                      <button
                ...
                <div className="pt-3 border-t border-uber-gray-100 flex items-center justify-between text-[10px] text-uber-gray-400 font-medium">
                  <span>ID Solicitud: {req.id.substring(0, 8)}...</span>
                  <span>
                    Solicitada el {new Date(req.createdAt).toLocaleDateString('es', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
              </div>
            );
      ...
      {/* ═══ CUSTOM MODAL: CANCEL REQUEST CONFIRMATION ═══ */}
      {cancelRequestId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md bg-white rounded-3xl overflow-hidden shadow-uber-lg animate-slide-up-mobile">
            ...
            <div className="bg-black text-white px-6 py-5 shrink-0">
              <h3 className="text-lg font-bold">¿Cancelar solicitud?</h3>
              <p className="text-xs text-uber-gray-400 mt-0.5">Esta acción retirará tu interés en este viaje</p>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-sm font-bold text-black">¿Confirmas la cancelación de la solicitud?</h4>
                  <p className="text-xs text-uber-gray-500 mt-1 leading-relaxed">
                    Retirarás tu solicitud para unirte a este viaje. Si decides viajar con este conductor más tarde, deberás enviar una nueva solicitud.
                  </p>
                </div>
              </div>
