...
import { UserController } from '../../src/modules/users/infrastructure/controllers/UserController';
...
describe('UserController Integration Tests', () => {
  ...
  let mockCreateVehicle: any;
  let mockGetVehicles: any;
  let mockDeleteVehicle: any;
  let mockUpdateVehicle: any;
  ...
  it('GET /profile/:userId - success', async () => {
    ...
    const res = await request(app).get('/profile/user-1');
  ...
  it('DELETE /vehicle - missing id', async () => {
    const res = await request(app).delete('/vehicle/');

    expect(res.status).toBe(404);
  });

  it('PUT /vehicle - missing id', async () => {
    const res = await request(app).put('/vehicle/').send({});
